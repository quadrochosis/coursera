...
; Do NOT change the two cases given to you, add more cases for other kinds
; of MUPL expressions.
; We will test eval-under-env by calling it directly even though, "in real
; life" it would be a helper function of eval-exp.
  ...
  (cond [(var? e)
        ...
        [(call? e)
         (let ([cls (eval-under-env (call-funexp e) env)]
           ...
           (if (not (closure? cls)) (error "MUPL call applied to non-closure")
               (letrec ([function (closure-fun cls)]
                        [fun_name (fun-nameopt function)]
                        [call_env (cons
                                   (cons (fun-formal function) arg)
                                   (closure-env cls))])
                 (eval-under-env (fun-body function)
                                 (if (string? fun_name)
                                     (cons (cons fun_name cls) call_env)
                                     call_env)))))]
        [(mlet? e)
         (let ([n (mlet-var e)]
...
; we will test this function directly,
; so it must do as described in the assignment
